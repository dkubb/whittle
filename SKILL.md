...
- You want to embed user-friendly localised error text. Whittle errors are
  machine-readable variants whose `Display` text is diagnostic, not
  contractual (see the rejection-message note under "Process for adding a
  new domain type"); localisation is the caller's concern.
...
## Least power: the weakest sufficient rule

Pick the *weakest* rule that captures the invariant exactly; extra
capability the invariant does not need is pure gap (review surface,
dependencies, opaque errors). The table maps recurring over-powered
choices to their weakest sufficient equivalent. Every row is
evidence-backed by dogfooding; none is speculative.

| Over-powered choice | Weakest sufficient |
|---|---|
| `Within<0, N>` on a carrier whose floor is `0` | `AtMost<N>` |
| `Pattern<RE>` over a finite nominal token set | `closed_set!` |
| `Pattern<RE>` for length + character class | `LenChars` + `EachChar<class>` |
| `LenBytes<MIN, MAX>` as the default bound | `LenChars<MIN, MAX>` |
| Narrow grammar on received provider text | `BoundedLine` / `BoundedText` |
| `Refined<u16, Within<1, 0xFFFF>>` | `NonZeroU16` |
| `Refined<String, …>` carrying fixed tokens | plain enum via `closed_set!` |
| A custom `Rule` impl | a primitive composition |

Row by row:

- **`Within<0, N>` → `AtMost<N>`** — the lower bound adds nothing the
  carrier doesn't already prove; state only what the invariant states.
- **`Pattern<RE>` → `closed_set!`** (finite nominal sets) — regex
  capability is pure gap for a finite set: `NoMatch` is opaque,
  members are not enumerable for generation or reject derivation, and
  it drags `regex` + nightly + `std` into a dependency-free invariant.
- **`Pattern<RE>` → `LenChars` + `EachChar<class>`** — `Pattern` is
  the escape hatch for **positional grammars only**; character-class
  rules compose, give character-precise errors, and stay `no_std`.
- **`LenBytes` → `LenChars`** — char count is THE length notion;
  `LenBytes` is the rare, explicitly-justified exception (the
  byte-length default was a real non-ASCII bug).
- **Narrow grammar → `BoundedLine` / `BoundedText`** — the provider
  owns its alphabet, so a narrow charset on received text is brittle.
  Tiers 1–2 of the provider-text policy (documented-opaque,
  undocumented) get length + broad printable class; documented
  discriminating structure (tier 3) is parsed explicitly; narrow
  grammars are tier 4 only (we-generate / contractually-guaranteed).
- **`Refined<u16, Within<1, 0xFFFF>>` → `NonZeroU16`** — don't compete
  with stdlib smart-ctor types: the wrapper adds proof surface and
  `unreachable!()` bridges at every stdlib↔`Refined` boundary without
  shrinking the state space.
- **`Refined<String, …>` → plain enum via `closed_set!`** — the enum
  is constructive (representable = admissible); a string carrier is
  predicative and forces `_ => unreachable!()` arms on every consumer.
- **Custom `Rule` → primitive composition** — primitives are
...
- **The admitted set is a finite nominal token set**: use
  ...
  carrier is involved.
- **The newtype is nominal-only** (distinction without validation):
  out of whittle's scope by design (IDEA §6 lists "a replacement
  for every Rust newtype" as a non-goal).
...
**Rejection-message text is NOT a stable contract.** The stable
error surface is the *variants* and their *categories*: a whittle
upgrade preserves the accept/reject set, the error variant, and its
payload fields (P5), but the `Display` strings — including the
rejection text serde surfaces at ingress — may be refined between
releases without notice. Consumers must branch on variants
(`StringError::BadChar { offset }`), never on rendered text, and
consumer tests must assert the exact variant, not the message
string. If a downstream contract appears to depend on message text,
that dependency is the bug to fix.
